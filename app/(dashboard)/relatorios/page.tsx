...
export default async function RelatoriosPage() {
    ...
    // Conteúdo gerencial otimizado para exportação e impressão
    const conteudoRelatorio = (
        <div className="space-y-6 text-stone-900 dark:text-stone-100">
            <div>
                <h2 className="text-xl font-black">{bar.nome}</h2>
                <p className="text-xs text-stone-500 dark:text-stone-400">
                    Relatório de Vendas e Extrato — Últimos 30 dias
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="border border-stone-200 dark:border-stone-800 p-3.5 rounded-xl bg-stone-50 dark:bg-stone-800/40">
                    <p className="text-[10px] uppercase font-bold text-stone-500">Itens Vendidos</p>
                    <p className="text-lg font-black">{totalItensVendidos} unidades</p>
                </div>
            </div>

            <div>
                <h3 className="text-xs font-bold uppercase tracking-wider mb-2 text-stone-700 dark:text-stone-300">
                    Ranking de Mais Vendidos
                </h3>
                <ul className="space-y-1.5 text-xs">
                    {rankingItens.map((item, idx) => (
                        <li key={item.nome} className="flex justify-between border-b border-stone-100 dark:border-stone-800 py-1.5">
                            <span>#{idx + 1} {item.nome} ({item.quantidade}x)</span>
                            <span className="font-bold">{formatarReais(item.faturamento)}</span>
                        </li>
                    ))}
                </ul>
            </div>
